...
class MainWindow(ctk.CTk):
    ...
    def __init__(self, fanctrlService):
        ...
        self.sidebar_frame.grid(row=0, column=0, sticky=ctk.NSEW)
        self.sidebar_frame.configure(fg_color="gray10", corner_radius=0)
        ...
        self.sidebar_title_label.pack(pady=(20, 0), padx=20, fill=ctk.X)
        ...
        self.button_1.pack(pady=(30, 0), padx=20, anchor=ctk.NW)

        self.sidebar_edition_label = ctk.CTkLabel(
            self.sidebar_frame,
            text="Edition",
            font=ctk.CTkFont(size=20, weight="bold"),
            anchor=ctk.W,
        )
        self.sidebar_edition_label.pack(pady=(30, 0), padx=20, fill=ctk.X)

        self.edition_list_label = ctk.CTkLabel(
            self.sidebar_frame,
            text="Strategy list",
            font=ctk.CTkFont(size=18),
            anchor=ctk.W,
        )
        self.edition_list_label.pack(pady=(10, 0), padx=40, fill=ctk.X)

        self.button_2 = ctk.CTkOptionMenu(
            self.sidebar_frame,
            values=["Strategy 1", "Strategy 2", "Strategy 3"],
            font=ctk.CTkFont(size=20, weight="bold"),
            anchor=ctk.W,
        )
        self.button_2.pack(pady=(10, 0), padx=40, anchor=ctk.NW)

        # self.button_2 = ctk.CTkButton(self.sidebar_frame, text="Button 2", command=self.button_2_action)
        # self.button_2.pack(pady=10, padx=10)

        self.main_frame = ctk.CTkFrame(self)
        self.main_frame.grid(row=0, column=1, columnspan=4, sticky=ctk.NSEW)
        self.main_frame.configure(fg_color="gray15", corner_radius=0)

        # FanPowerSliderBox(self.main_frame)
...
# from fw_fanctrl_gui.app.SystemTray import main as systemTrayMain
# from fw_fanctrl_gui.GCBypass import GCBYPASS
#
# class MainWindow:
